...
export const UnderReviewPropertiesScreen = () => {
  const router = useRouter();

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 12;

  const underReviewProperties = properties.filter(
    (property) => property.status === "under-review"
  );

  const totalPages = Math.ceil(underReviewProperties.length / itemsPerPage);

  const paginatedProperties = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = currentPage * itemsPerPage;

    return underReviewProperties.slice(startIndex, endIndex);
  }, [currentPage, underReviewProperties]);

  const viewProperty = (propertyId: string) => {
    router.push(`/properties/${propertyId}?status=under-review`);
  };
