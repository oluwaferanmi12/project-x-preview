...
import { getInitialsFromFullName, getMessageDay, getMessageTime } from "@repo/utils";
...
export function MessageDetail({ thread, onBack, isMobile = false }: MessageDetailProps) {
...
const handleSendMessage = () => {
  ...
  return (
    <Container className="flex flex-col h-full bg-surface">
      {/* Header */}
      <Container className="flex items-center justify-between border-b border-line px-5 py-2">
        <Container className="flex min-w-0 items-center gap-3">
          {isMobile && onBack && (
            <button
              type="button"
              onClick={onBack}
              className="rounded-lg p-2 transition-colors hover:bg-s50 lg:hidden"
              aria-label="Go back"
            >
              <ArrowLeft size={20} />
            </button>
        ...
        {thread.messages.map((message, index) => {
          ...
          return (
            <Container key={message.id}>
              ...
              >
                {!message.isOwn && (
                  <Container className="mr-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-p300">
                    <Text
                      variant="body-sm"
                      tone="inverted"
                      className="font-bold"
